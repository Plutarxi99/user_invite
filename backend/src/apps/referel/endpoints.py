...
from backend.src.apps.referel.schema import MyRefCod, RefCodSearch, RefererCod
from backend.src.apps.user.crud import get_user, get_user_on_id
from backend.src.apps.user.schemas import UserSchema, UserInfoReferal, UserBase, UserInfoReferalDoc
from backend.src.base.crud import CRUDBase, write_modeldb_in_modelschema
from backend.src.exceptions.model import ErrorResponseModel
from backend.src.models import User
from backend.src.operations.deps import get_current_active_user, get_db, create_user_schema, encode_referer_cod
...
async def get_my_referal_cod(
    ...
    # записываем в схему-модель и записываем в базу данных реферальный код пользователя
...
async def delete_my_ref_cod(
    ...
    if not current_user.referer_cod:
        raise ErrorResponseModel(code=401, message="У вас уже удален реферальный код")
    ref_cod = RefererCod(referer_cod=None)
    user = CRUDBase(model=User)
    user_not_ref_cod = user.update(db=db, obj_in=ref_cod, db_obj=user_db)
...
async def get_list_my_referals(
    ...
    list_referals = await get_referal_on_id(db=db, user_referer_id=user_referer_id)
    l_r = UserInfoReferalDoc(email_referer=user_referer.email, referals=list_referals)
    return l_r
...
async def get_referer_cod_on_email(
    ...
    if not referer:
        raise ErrorResponseModel(code=403, message="Пользователь с такой почтой не существует")
    ref_cod = referer.referer_cod
    if not ref_cod:
        raise ErrorResponseModel(code=403, message="Пользователь с такой почтой не имеет реферального кода")
...
async def get_referals_in_id_referer(
    ...
    l_r = UserInfoReferalDoc(email_referer=referer.email, referals=list_referals)
    return l_r
